...
export default function PortfolioPage() {
  ...
  const canCreate = user && ['super_admin', 'teacher', 'adviser', 'officer', 'member'].includes(user.role);
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0b0505] text-[#F8F2EC]">Loading portfolio...</div>;
  }

  if (!user) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0b0505] text-[#F8F2EC]">Please log in.</div>;
  }

  if (!canView) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0b0505] text-[#F8F2EC]">Your role does not have access to the portfolio gallery.</div>;
  }
    ...
    <div className="min-h-screen bg-[#0b0505] px-4 py-8 text-[#F8F2EC] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
          ...
          <div className="relative grid items-center gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3">
                {canCreate && (
                  <button onClick={() => setIsModalOpen(true)} className="rounded-full bg-gradient-to-r from-red-600 to-red-800 px-5 py-3 text-xs font-black uppercase tracking-[0.2em] text-[#F8F2EC] transition-transform hover:-translate-y-0.5 active:scale-95">
                    Submit Work
                  </button>
                )}
                <button onClick={() => window.scrollTo({ top: document.body.scrollHeight * 0.3, behavior: 'smooth' })} className="rounded-full border border-white/15 bg-white/[0.04] px-5 py-3 text-xs font-black uppercase tracking-[0.2em] text-[#F8F2EC] transition-colors hover:border-red-500/40 hover:text-red-200">
                  Browse Gallery
                </button>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                {[['40+', 'Student Works'], ['5', 'Creative Tracks'], ['Live', 'Showcase Feed']].map(([value, label]) => (
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.04] p-4 shadow-[0_25px_60px_rgba(0,0,0,0.45)] backdrop-blur-2xl">
          <div className="flex flex-wrap gap-3">
            {[{ id: 'all', label: 'All Works', count: entries.length }, { id: 'my_posts', label: 'My Submissions', count: myEntries.length }].map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`rounded-full px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${activeTab === tab.id ? 'bg-gradient-to-r from-red-600 to-red-800 text-[#F8F2EC]' : 'border border-white/10 bg-[#1a0d0d] text-[rgba(248,242,236,0.7)]'}`}>
        ...
        {displayedEntries.length > 0 ? (
          <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {displayedEntries.map((entry) => (
              <article key={entry._id} className="group overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] shadow-[0_25px_60px_rgba(0,0,0,0.45)] transition-transform duration-200 hover:-translate-y-1">
